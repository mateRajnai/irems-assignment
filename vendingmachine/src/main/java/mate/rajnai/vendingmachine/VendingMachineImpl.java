...
public class VendingMachineImpl implements VendingMachine {
	...
	private final Inventory<Product> availableProducts = new Inventory<Product>();
	private final Inventory<Coin> availableCoins = new Inventory<Coin>();
	private final Inventory<Coin> insertedCoinsOfCurrentPurchase = new Inventory<Coin>();
	...
	public Purchase buyProductAndReturnChangesIfAny(Product product) {
		...
		if(this.availableProducts.removeItem(product)) {
			...
			List<Coin> coinsAsChange = this.collectChange(changeAmount, product);
	...
	private List<Coin> collectChange(int changeAmount, Product product) {
		...
		while (changeAmount > 0) {
			if (changeAmount >= Coin.QUARTER.getValue() && this.availableCoins.hasItem(Coin.QUARTER)) {
				...
				throw new VendingMachineHasNotEnoughChangeException("Not enough change.");
			}	
		}
